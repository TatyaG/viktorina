...
export const labyrinth = () => {
  ...
  labyrinthWrap.append(canvasLab, faceImg, mazeImg);

  //ЛОГИКА ЛАБИРИНТА

  window.onload = function () {
    let trajectory = [];
    let context = canvasLab.getContext("2d");
    ...
    canvasLab.addEventListener("mousemove", function (e) {
      mouseX = e.clientX - canvasLab.getBoundingClientRect().x;
      mouseY = e.clientY - canvasLab.getBoundingClientRect().y;
      if (isMouseDown) {
    ...
    canvasLab.addEventListener("mousedown", function () {
      isMouseDown = true;
    });

    // Обработка окончания движения мыши
    canvasLab.addEventListener("mouseup", function () {
      isMouseDown = false;
    });

    // Обработка выхода за пределы области холста
    canvasLab.addEventListener("mouseleave", function () {
      isMouseDown = false;
    });
    ...
    canvasLab.addEventListener("mousemove", processMouseMove);
    canvasLab.addEventListener("touchmove", processTouchMove);

    // Обработка начала движения мыши и тача
    canvasLab.addEventListener("mousedown", function () {
      isMouseDown = true;
    });
    canvasLab.addEventListener("touchstart", function () {
      isMouseDown = true;
    });
    ...
    canvasLab.addEventListener("touchend", function () {
      isMouseDown = false;
    });
    ...
    function drawMaze() {
      mazeImg.onload = function () {
        canvasLab.width = mazeImg.width;
        canvasLab.height = mazeImg.height;
    ...
    function processMouseMove(e) {
      if (isMouseDown) {
        handleMovement(
          e.clientX - canvasLab.getBoundingClientRect().left - 15,
          e.clientY - canvasLab.getBoundingClientRect().top - 20
        );
      }
    }
    ...
    function processTouchMove(e) {
      if (isMouseDown) {
        ...
        handleMovement(
          touch.clientX - canvasLab.getBoundingClientRect().left - 15,
          touch.clientY - canvasLab.getBoundingClientRect().top - 20
        );
      }
    }
    ...
    function handleMovement(newX, newY) {
      ...
      if (
        ...
        // Очищаем холст
        context.clearRect(0, 0, canvasLab.width, canvasLab.height);
        ...
        if (y > canvasLab.height - faceImg.height) {
    ...
    function checkForCollision(newX, newY) {
      ...
      const collisionAreaSize = 10; // Размер области
      let checkArea = context.getImageData(
        newX - collisionAreaSize / 2,
        newY - collisionAreaSize / 2,
